import 'package:demo_dua_prooject/core/external_libs/lru_map/lru_map.dart';
import 'package:demo_dua_prooject/core/utility/logger_utility.dart';
import 'package:demo_dua_prooject/core/utility/trial_utility.dart';
import 'package:flutter/material.dart';
import 'package:intl/intl.dart';

final Map<Object, String> _translateNumberToBanglaStringCache =
    LruMap(maximumSize: 100);
...
bool hasOnlyBanglaOrEnglishNumber(String text) {
  final bool? onlyBanglaOrEnglishNumber = catchAndReturn(() {
    ...
    return !text.split('').any(
          (char) =>
              !(bengaliDigits.hasMatch(char) || englishDigits.hasMatch(char)),
        );
  });
...
String formatLocalTimeOfDayInBangla(TimeOfDay time) {
  final String? localTimeInBangla = catchAndReturn(() {
    ...
    final String formattedHour =
        translateNumberToBanglaString(hour).padLeft(2, '০');
    final String formattedMinute =
        translateNumberToBanglaString(minute).padLeft(2, '০');
...
DateTime convertTimeOfDayToExpectedDateTime(
  ...
  final DateTime? dateTime = catchAndReturn(() {
    final int addableDay =
        (addedDayCount <= 30 ? addedDayCount : (addedDayCount - 30)) - 1;
...
String removeHtml(String textWithHtml) =>
    Bidi.stripHtmlIfNeeded(textWithHtml.trim()).replaceAll("</highlig", "");
